...
tokenizer = AutoTokenizer.from_pretrained(
    "nguyenvulebinh/vi-mrc-base", use_auth_token=AUTH_TOKEN
)
...
class PairwiseModel(nn.Module):
    ...
    def forward(self, ids, masks):
        out = self.model(
            input_ids=ids, attention_mask=masks, output_hidden_states=False
        ).last_hidden_state
        out = out[:, 0]
        outputs = self.fc(out)
        return outputs
    ...
    def stage1_ranking(self, question, texts):
        ...
        valid_dataset = SiameseDatasetStage1(
            tmp, tokenizer, self.max_length, is_test=True
        )
    ...
    def stage2_ranking(self, question, answer, titles, texts):
        ...
        tmp["answer"] = answer
        tmp["title"] = titles
        valid_dataset = SiameseDatasetStage2(
            tmp, tokenizer, self.max_length, is_test=True
        )
        valid_loader = DataLoader(
            valid_dataset,
...
class SiameseDatasetStage1(Dataset):
    ...
    def __init__(self, df, tokenizer, max_length, is_test=False):
        ...
        self.content1 = tokenizer.batch_encode_plus(
            list(df.question.values), max_length=max_length, truncation=True
        )["input_ids"]
        self.content2 = tokenizer.batch_encode_plus(
    ...
    def __getitem__(self, index):
        return {
            ...
            "target": (
                torch.tensor(0)
                if self.is_test
                else torch.tensor(self.targets[index], dtype=torch.float)
            ),
        }
...
class SiameseDatasetStage2(Dataset):
    ...
    def __init__(self, df, tokenizer, max_length, is_test=False):
        ...
        self.tokenizer = tokenizer
        self.is_test = is_test
        self.df["content1"] = self.df.apply(
            lambda row: row.question + f" {tokenizer.sep_token} " + row.answer, axis=1
        )
        self.df["content2"] = self.df.apply(
            lambda row: row.title + f" {tokenizer.sep_token} " + row.candidate, axis=1
        )
        self.content1 = tokenizer.batch_encode_plus(
            list(df.content1.values), max_length=max_length, truncation=True
        )["input_ids"]
        self.content2 = tokenizer.batch_encode_plus(
            list(df.content2.values), max_length=max_length, truncation=True
        )["input_ids"]
        if not self.is_test:
            self.targets = self.df.label
